...
class ChatGPTOptimized(_PluginBase):
    # 插件名称
    ...
    # 作者主页
    author_url = "https://github.com/0xlane"
    # 插件配置项ID前缀
    plugin_config_prefix = "chatgpt_optimized_"
    # 加载顺序
    plugin_order = 15
    # 可使用的用户级别
    auth_level = 1
    ...
    _notify = False
    # 自定义提示词
    _customize_prompt = '''接下来我会给你一个电影或电视剧的文件名，你需要识别文件名中的名称、版本、段/节、年份、分辨率、第几季、第几集等信息，并按以下JSON格式返回：

{"name":string,"version":string,"part":string,"year":string,"resolution":string,"season":number|null,"episode":number|null}

特别注意：

1. 返回结果需要严格附合JSON格式，不需要有任何其它的字符。
2. 如果中文电影或电视剧的文件名中存在谐音字或字母替代的情况，请还原最有可能的结果。
3. 名字应省略“特别版”、“完整版”等描述，尽可能保证媒体名称真实可靠。'''
    ...
    def get_form(self) -> Tuple[List[dict], Dict[str, Any]]:
        ...
        return [
            ...
            "openai_key": "",
            "model": "gpt-3.5-turbo",
            "customize_prompt": '''接下来我会给你一个电影或电视剧的文件名，你需要识别文件名中的名称、版本、段/节、年份、分辨率、第几季、第几集等信息，并按以下JSON格式返回：
...
3. 名字应省略“特别版”、“完整版”等描述，尽可能保证媒体名称真实可靠。'''
    ...
    @eventmanager.register(EventType.UserMessage, 20)
